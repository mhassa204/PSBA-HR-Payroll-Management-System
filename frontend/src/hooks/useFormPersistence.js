...
export const useFormPersistence = ({
  ...
  useEffect(() => {
    ...
    if (isMountedRef.current && currentLocationRef.current !== location.pathname) {
  ...
  const saveFormData = useCallback(async () => {
    if (!enabled || isRestoringRef.current || !isMountedRef.current || isNavigatingRef.current) return;
    ...
    try {
      ...
      if (additionalState.districtMap && formValues.district_id) {
        dropdownLabels.district_label = additionalState.districtMap[String(formValues.district_id)] || '';
      }
      if (additionalState.cityMap && formValues.city_id) {
        dropdownLabels.city_label = additionalState.cityMap[String(formValues.city_id)] || '';
      }
      if (additionalState.levelMap) {
        // Store education level labels
        dropdownLabels.education_level_labels = {};
        if (additionalState.educations && Array.isArray(additionalState.educations)) {
          additionalState.educations.forEach(edu => {
            if (edu.education_level_id) {
              dropdownLabels.education_level_labels[edu.id] = {
                ...
                label: edu.education_level || additionalState.levelMap[String(edu.education_level_id)] || ''
              };
            }
          });
        }
      ...
      if (additionalState.uploadedFiles) {
        for (const [key, value] of Object.entries(additionalState.uploadedFiles)) {
          if (value === null || value === undefined) {
            ...
            // Array of files
            const fileArray = await Promise.all(value.map(file => fileToBase64(file)));
            processedFiles[key] = fileArray;
          } else if (typeof value === 'object') {
            // Object with file values (e.g., experience_documents, education_documents)
      ...
      if (additionalState.profilePicturePreview) {
  ...
  const restoreFormData = useCallback(async (options = {}) => {
    ...
    try {
      ...
      if (parsed.dropdownLabels) {
        if (parsed.dropdownLabels.district_label && additionalState.setDistrictMap) {
        ...
        if (parsed.dropdownLabels.education_level_labels && additionalState.setLevelMap) {
      ...
      // Restore experiences
      if (parsed.experiences && Array.isArray(parsed.experiences) && additionalState.setExperiences) {
        additionalState.setExperiences(parsed.experiences);
      }

      // Restore educations (after levelMap is updated so labels are available)
      if (parsed.educations && Array.isArray(parsed.educations) && additionalState.setEducations) {
        ...
        const updatedEducations = parsed.educations.map(edu => {
          if (parsed.dropdownLabels?.education_level_labels?.[edu.id]) {
            const savedLabel = parsed.dropdownLabels.education_level_labels[edu.id];
        ...
        try {
          ...
          for (const [key, value] of Object.entries(parsed.uploadedFiles)) {
            try {
              ...
              const shouldRestore = !currentFiles[key] || options.forceRestore;
              ...
              if (value === null || value === undefined) {
                ...
                const existingArray = Array.isArray(currentFiles[key]) ? currentFiles[key] : [];
                const fileArray = value
                  .map(fileData => {
                    try {
                      return base64ToFile(fileData);
                    } catch (e) {
                      console.warn(`Failed to restore file in array ${key}:`, e);
                      return null;
                    }
                  })
                  .filter(f => f !== null);
                ...
                fileArray.forEach(newFile => {
                  const exists = mergedArray.some(existingFile => 
                    existingFile instanceof File && newFile instanceof File &&
                ...
                const existingObject = typeof currentFiles[key] === 'object' && currentFiles[key] !== null 
                  ? currentFiles[key] 
                  : {};
                const restoredObject = { ...existingObject };
                for (const [objKey, objValue] of Object.entries(value)) {
                  try {
                    if (objValue && typeof objValue === 'object' && objValue.data) {
                    ...
                    console.warn(`Failed to restore file in object ${key}.${objKey}:`, e);
                  }
                }
      ...
      if (parsed.profilePictureData && additionalState.setProfilePicturePreview) {
        if (parsed.profilePictureData.isUrl && parsed.profilePictureData.preview) {
          // Restore from URL
          additionalState.setProfilePicturePreview(parsed.profilePictureData.preview);
      ...
      } else if (parsed.profilePicturePreview && additionalState.setProfilePicturePreview) {
        additionalState.setProfilePicturePreview(parsed.profilePicturePreview);
      }
        ...
        if (districtId) {
          ...
          setTimeout(() => {
            ...
            if (currentDistrictId === districtId) {
              ...
              formData.setValue("district_id", districtId, { shouldDirty: false });
            }
          }, 100);
        }
      }, 500);
  ...
  useEffect(() => {
    ...
    const subscription = formData.watch(() => {
      ...
      saveTimeoutRef.current = setTimeout(() => {
        ...
        if (isMountedRef.current && !isRestoringRef.current && !isNavigatingRef.current) {
          saveFormData();
        }
  ...
  useEffect(() => {
    ...
    const filesChanged = prevFiles === null || 
      JSON.stringify(Object.keys(currentFiles || {}).sort()) !== JSON.stringify(Object.keys(prevFiles || {}).sort()) ||
      Object.keys(currentFiles || {}).some(key => {
        ...
        if (current instanceof File && prev instanceof File) {
          return current.name !== prev.name || current.size !== prev.size || current.lastModified !== prev.lastModified;
        }
