...
const LeaveTypesDialog = ({ open, onClose }) => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get("/leave-banks/types");
      setTypes(data.types || []);
    } catch {
    } finally {
      setLoading(false);
  ...
  const add = async (e) => {
    ...
    if (!form.name) return;
    try {
      await axios.post("/leave-banks/types", form);
      setForm({ name: "", code: "" });
      await load();
      toastBus.emit({ type: "success", message: "Leave type added" });
    } catch (e) {
      toastBus.emit({
        type: "error",
  ...
  const update = async (id, patch) => {
    try {
      await axios.put(`/leave-banks/types/${id}`, patch);
      await load();
    } catch (e) {
      toastBus.emit({
        type: "error",
  ...
  const remove = async (id) => {
    try {
      await axios.delete(`/leave-banks/types/${id}`);
      await load();
    } catch (e) {
      toastBus.emit({
        type: "error",
      ...
      <div className="modal-surface w-full max-w-2xl max-h-[85vh] overflow-y-auto custom-thin-scroll">
        ...
        <div className="p-4 space-y-4">
          <form className="flex flex-wrap gap-2" onSubmit={add}>
            <input
              className="form-input !w-48 !py-1 !px-2 text-xs"
              placeholder="Name"
              value={form.name}
              onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            />
            <input
              className="form-input !w-40 !py-1 !px-2 text-xs"
                      ...
                      <tr key={t.id}>
                        ...
                        <td>
                          <input
                            ...
                            value={t.code || ""}
                            onChange={(e) =>
                              update(t.id, { code: e.target.value })
                            }
                          />
                        </td>
                        <td>
...
const EditAllocationsDialog = ({ open, onClose, bank }) => {
  ...
  const load = async () => {
    try {
      ...
      const [{ data: t }, { data: a }] = await Promise.all([
        ...
        axios.get(`/leave-banks/${bank.id}/allocations`, {
          params: { search },
        }),
      ]);
      setTypes(t.types || []);
  ...
  useEffect(() => {
    if (open && bank?.id) load();
  }, [open, bank?.id]);

  const getEmpAlloc = (empId, typeId) =>
    allocs.find((x) => x.employee_id === empId && x.leave_type_id === typeId)
      ?.days || 0;
  const setEmpAlloc = (empId, typeId, days) => {
    setAllocs((prev) => {
      const idx = prev.findIndex(
        (x) => x.employee_id === empId && x.leave_type_id === typeId
      );
      const next = [...prev];
      if (idx >= 0) next[idx] = { ...next[idx], days };
      else
        next.push({
          leave_bank_id: bank.id,
          employee_id: empId,
      ...
      <div className="modal-surface w-full max-w-6xl max-h-[92vh] overflow-y-auto custom-thin-scroll">
        <div className="modal-header">
          <h2 className="text-sm font-semibold tracking-wide">
            Edit Leave Allocations - {bank.title || "#" + bank.id}
          </h2>
          <button onClick={onClose} className="btn btn-outline text-xs">
            Close
          </button>
        ...
        <div className="p-4 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <input
              className="form-input !w-64 !py-1 !px-2 text-xs"
              placeholder="Search by CNIC/Name/Emp ID"
              value={search}
                        ...
                        {types.map((t) => (
                          <td key={t.id}>
                            <input
                              type="number"
                              min={0}
                              className="form-input !py-1 !px-2 w-24 text-xs"
                              value={getEmpAlloc(emp.id, t.id)}
                              onChange={(e) =>
                                setEmpAlloc(emp.id, t.id, e.target.value)
                              }
                            />
                          </td>
                        ))}
          ...
          <div>
            <button className="btn btn-success text-xs" onClick={save}>
              Save
            </button>
          </div>
        </div>
...
const EditBankDialog = ({ open, onClose, bank }) => {
  ...
  const load = async () => {
    try {
      ...
      setTypes(t.types || []);
      const map = {};
      (d.defaults || []).forEach((it) => {
        map[it.leave_type_id] = it.days;
      });
      setDefaults(map);
    } catch {}
  };
  ...
  useEffect(() => {
    if (open && bank) {
      setForm({
        title: bank.title || "",
        start: (bank.period_start || "").slice(0, 10),
        end: (bank.period_end || "").slice(0, 10),
      });
      load();
    }
  }, [open, bank]);
  ...
  const save = async () => {
    try {
      await axios.put(`/leave-banks/${bank.id}`, {
        title: form.title,
        period_start: form.start,
        period_end: form.end,
      });
      const items = Object.entries(defaults).map(([leave_type_id, days]) => ({
        leave_type_id: Number(leave_type_id),
        days: Number(days || 0),
      }));
      if (items.length)
        await axios.post(`/leave-banks/${bank.id}/defaults`, { items });
              ...
              <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-3">
                {types.map((t) => (
                  <div key={t.id} className="flex items-center gap-2">
                    <label className="w-40 text-[11px] text-gray-700">
                      {t.name}
                    </label>
                    <input
                      type="number"
                ...
                {!types.length && (
                  <div className="text-xs text-gray-500">No leave types</div>
                )}
              </div>
            </div>
          </div>
...
const LeaveBankPage = () => {
  ...
  const toggleDefault = (typeId, days) =>
    setDefaultDays((prev) => ({ ...prev, [typeId]: days }));

  const createBank = async () => {
    try {
      if (!createForm.start || !createForm.end) return;
      const defaults = Object.entries(defaultDays).map(
        ([leave_type_id, days]) => ({ leave_type_id, days: Number(days || 0) })
      );
      const { data } = await axios.post("/leave-banks", {
        title: createForm.title,
        period_start: createForm.start,
        period_end: createForm.end,
        defaults,
      });
  ...
  const deleteBank = async (id) => {
    if (
      !window.confirm("Delete this leave bank? This will hide it from lists.")
    )
      return;
    try {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="card-soft p-5 space-y-5">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <input
            className="form-input !w-56 !py-1 !px-2 text-xs"
            ...
            {types.map((t) => (
              <div key={t.id} className="flex items-center gap-2">
                ...
                <input
                  ...
                  min={0}
                  className="form-input !py-1 !px-2 w-24 text-xs"
                  value={defaultDays[t.id] || ""}
                  onChange={(e) => toggleDefault(t.id, e.target.value)}
                />
              </div>
            ))}
            {!types.length && (
              <div className="text-xs text-gray-500">
                No leave types. Add some using the button above.
              </div>
            )}
          </div>
          <div>
            <button className="btn btn-success text-xs" onClick={createBank}>
              Create Leave Bank
            </button>
          </div>
        </div>
            ...
            {banks.map((b) => (
              <tr key={b.id}>
                ...
                <td className="px-3 py-2 text-sm">
                  {b.period_start?.slice(0, 10)} to {b.period_end?.slice(0, 10)}
                </td>
                <td className="px-3 py-2 text-sm text-left">
                  {(b.defaults || []).map((d) => (
                    <div key={d.id} className="text-[10px] text-gray-700">
                      {d.leaveType?.name}: {d.days}
                    </div>
                  ))}
                  {!b.defaults?.length && (
