...
import {
  ...
  createTravelClaim,
  updateTravelClaim,
  /*uploadClaimReceipts,*/ updateClaimItem,
  deleteClaimItem,
  uploadClaimItemReceipts,
...
export default function TravelClaimsPage() {
  ...
  const addItem = () => {
    ...
    setForm((prev) => ({
      ...prev,
      items: [...prev.items, { ...item, attachments: [] }],
    }));
  ...
  const addDraftAttachments = (idx, files) => {
    ...
    setForm((prev) => ({
      ...
      items: prev.items.map((it, i) =>
        i === idx
          ? { ...it, attachments: [...(it.attachments || []), ...arr] }
          : it
      ),
    }));
  };
    ...
    setForm((prev) => ({
      ...
      items: prev.items.map((it, i) =>
        i === idx
          ? {
              ...it,
              attachments: (it.attachments || []).filter((_, j) => j !== fIdx),
            }
          : it
      ),
    }));
  ...
  const onUpdateAmount = async (itemId, amount) => {
    if (!selected) return;
    const updated = await updateClaimItem(selected.id, itemId, {
                        ...
                        <div key={idx} className="p-2 text-sm">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <label className="px-2 py-1 rounded bg-accent/40 border cursor-pointer text-xs">
                                Upload Receipts
                                <input
                                  type="file"
                                  multiple
                                  ...
                                  onChange={(e) => {
                                    if (e.target.files?.length) {
                                      addDraftAttachments(idx, e.target.files);
                                      e.target.value = "";
                                    }
                                  }}
                                />
                                ...
                                return (
                                  <div key={fIdx} className="relative">
                                    {isImg ? (
                                      ...
                                      <a
                                        className="inline-flex items-center px-2 py-1 text-xs border rounded bg-white text-primary hover:underline"
                                        href={url}
                                        target="_blank"
                                        rel="noreferrer"
                                      >
                                    ...
                                    <button
                                      title="Remove"
                                      onClick={() =>
                                        removeDraftAttachment(idx, fIdx)
                                      }
        ...
        <CardContent className="p-0">
          <div className="divide-y">
            ...
            {!loading && list.length === 0 && (
              <div className="p-6 text-muted-foreground">No claims found</div>
            )}
            {list.map((r) => (
              <div
                key={r.id}
              ...
              >
                <div>
                  ...
                  <div className="text-muted-foreground">
                    Items: {r.items?.length || 0} · Total Claimed:{" "}
                    {r.total_claimed}
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="pt-2">
                ...
                <div className="mt-2 rounded-lg border divide-y">
                  ...
                  {(selected.items || []).map((it, idx) => (
                    <div
                      key={it.id}
                      className="p-3 grid grid-cols-12 gap-3 items-center text-sm"
                    >
                      ...
                      <div className="col-span-2">
                        ...
                        {can("travel.claim.update") ? (
                          <input
                            ...
                            onBlur={(e) =>
                              onUpdateAmount(it.id, e.target.value)
                            }
                          />
                        ) : (
                      ...
                      <div className="col-span-12 md:col-span-12 lg:col-span-12">
                        <div className="flex items-center justify-between">
                          ...
                          {can("travel.claim.update") && (
                            <label className="px-2 py-1 rounded bg-emerald-600 text-white cursor-pointer text-xs">
                              Add Receipts
                              <input
                                type="file"
                                multiple
                                ...
                                onChange={async (e) => {
                                  if (e.target.files?.length) {
                                    const claim = await uploadClaimItemReceipts(
                                      selected.id,
                                      it.id,
                                      e.target.files
                                    );
                            ...
                            <div key={r.id} className="relative">
                              ...
                              {can("travel.claim.update") && (
                                <button
                                  title="Remove"
                                  onClick={async () => {
                                    const claim = await deleteClaimItemReceipt(
                                      selected.id,
                                      it.id,
                                      r.id
                                    );
                  ...
                  <div className="p-3 text-right text-sm font-medium">
                    Total Claimed: {sum(selected.items)}
                  </div>
                </div>
              </div>
            </>
