...
import {
  ...
  clearLastDecisionTravelRequest,
...
export default function ManageTravelRequests() {
  ...
  const locType = (r) =>
    r?.applicant?.employmentRecords?.[0]?.location?.type || "HEAD_OFFICE";
  ...
  const doAction = async (r, action) => {
    ...
    if (
      !window.confirm(
        `Confirm to ${action.toLowerCase().replace("_", " ")} request #${r.id}?`
      )
    )
      return;
    setSubmitting(true);
    try {
      if (action === "RECOMMEND") await recommendTravelRequest(r.id);
        ...
        await clearLastDecisionTravelRequest(r.id);
                ...
                <div className="flex items-center gap-1">
                  {canUndoRecommendation(selected) && (
                    <Button
                      ...
                      size="sm"
                      variant="secondary"
                      onClick={() => doAction(selected, "UNDO_RECOMMEND")}
                    >
                      Undo
                    </Button>
                  )}
                  ...
                  {canUndoDecision(selected) && (
                    <Button
                      ...
                      size="sm"
                      variant="secondary"
                      onClick={() => doAction(selected, "UNDO_LAST")}
                    >
                      Undo
                    </Button>
                  )}
