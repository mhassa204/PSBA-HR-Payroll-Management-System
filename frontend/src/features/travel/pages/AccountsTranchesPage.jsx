...
import {
  listVerifiedClaimsForAccounts,
  createExpenseTranche,
  listExpenseTranches,
...
export default function AccountsTranchesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const rs = await listVerifiedClaimsForAccounts(filters);
      setClaims(rs);
    } finally {
      setLoading(false);
    }
  ...
  const loadTranches = async () => {
    ...
    try {
      const rs = await listExpenseTranches();
      setTranches(rs);
    } finally {
  ...
  const makeTranche = async () => {
    if (selectedIds.length === 0) {
      alert("Select claims to group");
      return;
    }
    if (
      !window.confirm(
        `Create tranche with ${selectedIds.length} claim(s) and mark them Processed?`
      )
    )
      return;
    try {
      const t = await createExpenseTranche({
        title,
        notes,
      ...
      await load();
      await loadTranches();
      alert(`Tranche ${t.code || t.id} created`);
    } catch (e) {
  ...
  return (
    <div className="space-y-4 w-full">
        ...
        <CardHeader className="border-b">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground hidden md:inline">
              Only claims where processing has been started by Accounts are
              listed here
            </span>
          </div>
        </CardHeader>
        <CardContent className="p-4 text-sm space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div>
              ...
              <Input
                value={filters.employee_cnic}
                onChange={(e) =>
                  setFilters((f) => ({ ...f, employee_cnic: e.target.value }))
                }
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                type="date"
                value={filters.claim_from}
                onChange={(e) =>
                  setFilters((f) => ({ ...f, claim_from: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                type="date"
                value={filters.claim_to}
                onChange={(e) =>
                  setFilters((f) => ({ ...f, claim_to: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="rounded border max-h-[60vh] overflow-auto divide-y">
            ...
            {!loading && claims.length === 0 && (
              <div className="p-3 text-muted-foreground">
                No claims with processing started match filters.
              </div>
            )}
            {claims.map((c) => {
              const emp = c.employee;
              const job = emp?.employmentRecords?.[0];
              const hasAccountsApproval = (c.statusEntries || []).some(
                (se) => se.action === "PROCESS_STARTED"
              );
              return (
                <div
                  key={c.id}
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-medium truncate">
                        Claim #{c.id} • {emp?.full_name || "Employee"}{" "}
          ...
          <div className="grid md:grid-cols-3 gap-3 mt-4">
            <div>
              ...
              <Input
                ...
                placeholder="e.g., Sep-2025 Batch 1"
              />
            </div>
      ...
      <Card className="w-full">
        <CardHeader className="border-b">
          <CardTitle>Processed Tranches</CardTitle>
        </CardHeader>
        <CardContent className="p-4 text-sm space-y-3">
          {loadingTranches && (
            <div className="text-muted-foreground">Loading...</div>
          )}
          {!loadingTranches && tranches.length === 0 && (
            <div className="text-muted-foreground">No tranches yet.</div>
          )}
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => exportTranche(t.id)}
                    >
                      Export CSV
                    </Button>
                    <Button size="sm" onClick={() => exportTrancheToPdf(t)}>
                      Export PDF
                    </Button>
                  </div>
                </div>
