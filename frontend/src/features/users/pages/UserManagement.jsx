          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-lg" style={{ color: "var(--color-text-secondary)" }}>
                {isCreateRoute ? 'Add a new user to the system' : 'Manage system users and their access'}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="p-6">
              {users.length === 0 ? (
                <div className="text-center py-12">
                  <Users className="w-16 h-16 mx-auto mb-4 opacity-50" style={{ color: "var(--color-text-secondary)" }} />
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {users.map((user) => (
                        <tr key={user.id} className="border-b hover:bg-gray-50" style={{ borderColor: "var(--color-border)" }}>
                          ...
                          <td className="py-3 px-4">
                            {user.role ? (
                              <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                                ...
                                {isSuper ? user.role.name : (user.role.name === 'Super Admin' ? 'Admin' : user.role.name)}
                              </span>
                            ) : (
                          ...
                          {canManage && (
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => isSuper || user.role?.name !== 'Super Admin' ? handleEditUser(user) : null}
                                  disabled={!isSuper && user.role?.name === 'Super Admin'}
                                ...
                                <button
                                  onClick={() => isSuper || user.role?.name !== 'Super Admin' ? handleDeleteUser(user) : null}
                                  disabled={!isSuper && user.role?.name === 'Super Admin'}
