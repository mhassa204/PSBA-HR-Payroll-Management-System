...
const DepartmentForm = ({ department, onSubmit, onCancel }) => {
  ...
  const form = useForm({
    defaultValues: {
      ...
      head_employee_id: department?.head?.id ? String(department.head.id) : ''
    }
  });
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      try {
        const opts = await employeeService.getAllEmployeesForDropdown();
        setEmployeeOptions([{ value: '', label: '— None —' }, ...opts]);
      } catch (e) {
        console.error('Error loading employees for dropdown', e);
      }
    };
    loadEmployees();
  }, []);
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        head_employee_id: data.head_employee_id ? Number(data.head_employee_id) : null
      };
      if (department) {
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 p-6">
      ...
      <div className="px-2">
        ...
        <select
          ...
          defaultValue={department?.head?.id ? String(department.head.id) : ''}
        >
        ...
        <p className="mt-1 text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
          Optional: Assign an employee as the Head of this department
        </p>
      </div>
