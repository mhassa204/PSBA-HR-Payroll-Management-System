...
const ScaleGradeForm = ({ scaleGrade, onSubmit, onCancel, isSubmitting }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.name.trim()) {
      showError('Scale/Grade name is required');
      return;
    }
    
    if (formData.name.trim().length < 2) {
      showError('Scale/Grade name must be at least 2 characters long');
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-6">
      ...
      <div className="px-2">
        ...
        >
          Scale/Grade Name <span style={{ color: "var(--color-error-500)" }}>*</span>
        </label>
        <input
          type="text"
          id="name"
