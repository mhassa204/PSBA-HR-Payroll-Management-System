...
const LocationTab = ({
      ...
      >
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
          ...
          <p className="text-gray-600">
            Location information is not required for this organization.
          </p>
        </div>
      </motion.div>
    );
  ...
  const locationSelectOptions = filteredLocations.map((l) => ({
    value: l.id,
    label: `${l.name}${l.city?.name ? " - " + l.city.name : ""}${
      l.district?.name ? " (" + l.district.name + ")" : ""
    }`,
  }));
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {typeOptions.map((t) => (
                <button
                  ...
                  key={t.value}
                  onClick={() =>
                    locationForm.setValue("_location_type_filter", t.value)
                  }
            ...
            <div className={getFieldClasses("location", "location_id")}>
              ...
              <SearchableSelect
                ...
                onChange={(value) =>
                  locationForm.setValue("location_id", value)
                }
                placeholder="Choose a location"
                register={register}
                name="location_id"
                required={
                  getValidationRules("location", "location_id", {
              ...
              {locationErrors?.location_id && (
                <p className="text-red-600 text-sm mt-1">
                  {locationErrors.location_id.message}
                </p>
              )}
              <p className="text-xs text-gray-500 mt-1">
                Filter by type using the chips above. List shows active
                locations from master data.
              </p>
            </div>
          </div>
