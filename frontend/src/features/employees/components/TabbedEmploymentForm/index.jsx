...
const TabbedEmploymentForm = forwardRef(
  (
    ...
    const transformedInitialData = useMemo(() => {
      if (!editingRecord || (!isEditMode && !isCreatingFromExisting))
        return null;

      const isNestedFormat =
        editingRecord.employment ||
        editingRecord.salary ||
        editingRecord.location ||
        editingRecord.contract;
      let transformed;
      if (isNestedFormat) {
        transformed = {
          id: editingRecord.id,
        ...
        transformed = {
          ...
          employment: {
            ...
            is_current:
              editingRecord.is_current !== undefined
                ? editingRecord.is_current
                : false,
      ...
      if (transformed && !transformed.designation) {
        transformed.designation =
          editingRecord.designation ||
          transformed.employment?.designation ||
          "";
      }
      if (transformed && !transformed.department) {
        transformed.department =
          editingRecord.department || transformed.employment?.department || "";
      }
      if (transformed && !transformed.role_tag) {
        transformed.role_tag =
          editingRecord.role_tag || transformed.employment?.role_tag || "";
      }
      if (transformed && !transformed.scale_grade) {
        transformed.scale_grade =
          editingRecord.scale_grade ||
          transformed.employment?.scale_grade ||
          "";
      }
      if (transformed && !transformed.reporting_officer_id) {
        transformed.reporting_officer_id =
          editingRecord.reporting_officer_id ||
          transformed.employment?.reporting_officer_id ||
          "";
      }
      if (transformed && !transformed.is_current) {
        transformed.is_current =
          editingRecord.is_current !== undefined
            ? editingRecord.is_current
            : transformed.employment?.is_current !== undefined
            ? transformed.employment.is_current
            : false;
      }
    ...
    const handleCloneEmployment = async () => {
      try {
        if (!isEditMode || !savedEmploymentId) {
          alert(
            "Clone is only available while editing an existing employment record."
          );
          return;
        }
        if (
          !window.confirm(
            "Clone this employment as a new record and set it as current? The existing record will be marked not current."
          )
        ) {
          return;
        }
        ...
        const existing = await employmentService.getEmploymentById(
          savedEmploymentId
        );
        const employmentBase = existing?.employment || existing || {};
        const salaryBase =
          existing?.salary || existing?.employment?.salary || null;
        const contractBase =
          existing?.contract || existing?.employment?.contract || null;
        const locationBase =
          existing?.location || existing?.employment?.location || null;
        const renewalReportDoc = Array.isArray(existing?.documents)
          ? existing.documents.find((doc) => doc.file_type === "renewal_report")
          : null;
        ...
        const organization =
          eForm.organization || employmentBase.organization || "";
        const department =
          eForm.department ||
          employmentBase.department ||
          employmentBase.department_id ||
          "";
        const designation =
          eForm.designation ||
          employmentBase.designation ||
          employmentBase.designation_id ||
          "";
        const employment_type =
          eForm.employment_type || employmentBase.employment_type || "Regular";
        const role_tag =
          eForm.role_tag ||
          employmentBase.role_tag ||
          employmentBase.role_tag_id ||
          "";
        const effective_from =
          eForm.effective_from || employmentBase.effective_from || "";
        const effective_till = normalizeDateField(
          eForm.effective_till ?? employmentBase.effective_till ?? null
        );
        const filer_status =
          eForm.filer_status || employmentBase.filer_status || "non_filer";
        const remarks = eForm.remarks ?? employmentBase.remarks ?? "";
        const scale_grade =
          eForm.scale_grade ||
          employmentBase.scale_grade ||
          employmentBase.scale_grade_id ||
          "";
        const reporting_officer_id =
          eForm.reporting_officer_id ??
          employmentBase.reporting_officer_id ??
          "";
        ...
        if (
          ...
          const salaryPayload = {
            basic_salary:
              mergedSalary.basic_salary != null
                ? Number(mergedSalary.basic_salary)
                : 0,
            gross_salary:
              mergedSalary.gross_salary != null
                ? Number(mergedSalary.gross_salary)
                : 0,
            medical_allowance:
              mergedSalary.medical_allowance != null
                ? Number(mergedSalary.medical_allowance)
                : 0,
            house_rent:
              mergedSalary.house_rent != null
                ? Number(mergedSalary.house_rent)
                : 0,
            conveyance_allowance:
              mergedSalary.conveyance_allowance != null
                ? Number(mergedSalary.conveyance_allowance)
                : 0,
            other_allowances:
              mergedSalary.other_allowances != null
                ? Number(mergedSalary.other_allowances)
                : 0,
            daily_wage_rate:
              mergedSalary.daily_wage_rate != null
                ? Number(mergedSalary.daily_wage_rate)
                : 0,
        ...
        if (
          ...
          const contractPayload = {
            ...
            renewal_count:
              c.renewal_count != null ? Number(c.renewal_count) : 0,
            ...
            is_renewed:
              typeof c.is_renewed === "string"
                ? c.is_renewed === "true"
                : Boolean(c.is_renewed),
    ...
    const handleOrganizationChange = useCallback(
      (newOrganization) => {
        ...
        if (previousOrganization && previousOrganization !== newOrganization) {
          ...
          alert(
            `Organization changed from ${previousOrganization} to ${newOrganization}. All form data has been reset. Please fill in the form again.`
          );
        }
        setPreviousOrganization(newOrganization);
        setIsInitialLoad(false);
      ...
      try {
        ...
        if (confirmType === "employment") {
          ...
          result = await createLocationRecord({
            employment_id: savedEmploymentId,
            location_id: confirmData.location_id || null,
          });
          ...
          result = await createContractRecord({
            ...
            confirmation_status:
              confirmData.confirmation_status || "In Progress",
    ...
    const onEmploymentSubmit = async (data) => {
      try {
        ...
        const existingDocuments = documentManager.documents.filter(
          (doc) => !doc.isNewFile
        );
        const documentData = {};
        ...
        console.error(
          "❌ TabbedEmploymentForm: Error in employment form:",
          error
        );
    ...
    const onSalarySubmit = async (data) => {
      try {
        const salaryField =
          currentOrganization === "MBWO" ? "gross_salary" : "basic_salary";
        const salaryValue = data[salaryField];
        if (!salaryValue || salaryValue <= 0) {
          const fieldName =
            currentOrganization === "MBWO" ? "gross salary" : "basic salary";
    ...
    const onLocationSubmit = async (data) => {
      try {
        ...
        setCompletedTabs((prev) => ({ ...prev, location: true }));
        if (isContractual) setActiveTab("contract");
        else await handleCompleteForm();
      } catch (error) {
        console.error(
          "❌ TabbedEmploymentForm: Error in location form:",
          error
        );
    ...
    const onContractSubmit = async (data) => {
      try {
        ...
        console.error(
          "❌ TabbedEmploymentForm: Error in contract form:",
          error
        );
    ...
    const handleCompleteForm = async () => {
      try {
        ...
        const completeData = {
          ...
          conveyance_allowance: parseFloat(
        ...
        console.error(
          "❌ TabbedEmploymentForm: Error preparing form data:",
          error
        );
    ...
    const handleFinalSubmit = async () => {
      try {
        ...
        const structuredData = {
          renewal_report:
            filesToUpload.renewal_report || previewData.renewal_report || null,
          ...
          contract: {
            ...
            renewal_report:
              filesToUpload.renewal_report ||
              previewData.renewal_report ||
              null,
          },
        };
        setShowPreviewModal(false);
        if (onSubmit) {
          ...
          console.error(
            "❌ TabbedEmploymentForm: No onSubmit function provided"
          );
          setIsSubmitting(false);
        }
      } catch (error) {
        console.error(
          "❌ TabbedEmploymentForm: Error in final submission:",
          error
        );
    ...
    const displayValue = (value, type = "text") => {
      ...
      if (typeof value === "string" && value.trim() !== "") {
        ...
        const isDateString = datePatterns.some((pattern) =>
          pattern.test(value)
        );
    ...
    const handleFinishLater = () => {
      ...
      if (onSubmit && completedTabs.employment) {
        ...
        const locationData = completedTabs.location
          ? locationForm.getValues()
          : {};
        const contractData = completedTabs.contract
      ...
      const loadEmploymentData = async () => {
        if (
          ...
          try {
            ...
            const employmentData = await employmentService.getEmploymentById(
              editingRecord.id
            );
            if (employmentData) {
              ...
              const transformedData = {
                ...
                employment: {
                  ...
                  effective_till: employmentData.effective_till
                    ? employmentData.effective_till.split("T")[0]
                    : "",
                  reporting_officer_id:
                  ...
                  employment_status:
                    employmentData.employment_status || "active",
                  is_current: employmentData.is_current || false,
                  is_on_probation: employmentData.is_on_probation || false,
                  probation_end_date: employmentData.probation_end_date
                    ? employmentData.probation_end_date.split("T")[0]
                    : "",
                },
                salary: employmentData.salary
                  ? {
                      ...employmentData.salary,
                      salary_effective_from: employmentData.salary
                        .salary_effective_from
                        ? employmentData.salary.salary_effective_from.split(
                            "T"
                          )[0]
                        : "",
                      salary_effective_till: employmentData.salary
                        .salary_effective_till
                        ? employmentData.salary.salary_effective_till.split(
                            "T"
                          )[0]
                        : "",
                    }
                ...
                location: employmentData.location
                  ? { location_id: employmentData.location.id }
                  : {},
                contract: employmentData.contract
                  ? {
                      ...employmentData.contract,
                      start_date: employmentData.contract.start_date
                        ? employmentData.contract.start_date.split("T")[0]
                        : "",
                      end_date: employmentData.contract.end_date
                        ? employmentData.contract.end_date.split("T")[0]
                        : "",
                      probation_start: employmentData.contract.probation_start
                        ? employmentData.contract.probation_start.split("T")[0]
                        : "",
                      probation_end: employmentData.contract.probation_end
                        ? employmentData.contract.probation_end.split("T")[0]
                        : "",
                      confirmation_date: employmentData.contract
                        .confirmation_date
                        ? employmentData.contract.confirmation_date.split(
                            "T"
                          )[0]
                        : "",
                    }
              ...
              if (
                employmentData.documents &&
                employmentData.documents.length > 0
              ) {
        ...
        >
          ...
          >
            ...
            <div className="tab-content">
              ...
              {activeTab === "history" && (
                <HistoryTab employmentId={savedEmploymentId} />
              )}
            </div>
          </div>
        </EnhancedModal>
          ...
          <div className="p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                Are you sure you want to save this {confirmType} information?
                You can add more details later if needed.
              </p>
            </div>
            <div className="flex space-x-4">
              ...
              >
                <i className="fas fa-save mr-2"></i>Yes, Save
              </button>
            </div>
          </div>
        </EnhancedModal>
          ...
          <div className="p-6">
            ...
            <div className="flex space-x-4">
              <button
                ...
                className="flex-1 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium"
              >
                <i className="fas fa-times mr-2"></i>Finish Later
              </button>
              {((activeTab === "employment" && !completedTabs.salary) ||
                ...
                >
                  <i className="fas fa-arrow-right mr-2"></i>Continue to Next
                  Tab
                </button>
              )}
            </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      <span className="text-gray-600">Role Tag:</span>{" "}
                      <span className="ml-1 text-gray-900">
                        {getLabelFromId(
                    ...
                    <div>
                      <span className="text-gray-600">Scale Grade:</span>{" "}
                      <span className="ml-1 text-gray-900">
                        {getLabelFromId(
                    ...
                    <div>
                      <span className="text-gray-600">Effective Till:</span>{" "}
                      <span className="ml-1 text-gray-900">
                        {displayValue(previewData.effective_till)}
                      </span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="ml-1 text-gray-900">
                        {displayValue(previewData.reporting_officer_id)}
                      </span>
                    </div>
                  ...
                  {previewData.organization === "MBWO" ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.gross_salary, "currency")}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-600">Payment Mode:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.payment_mode)}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.basic_salary, "currency")}
                        </span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.house_rent, "currency")}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="ml-1 text-gray-900">
                          {displayValue(
                            previewData.conveyance_allowance,
                            "currency"
                          )}
                        </span>
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-gray-600">Type:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.contract_type)}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-600">Number:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.contract_number)}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-600">Start Date:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.contract_start_date)}
                        </span>
                      </div>
                      <div>
                      ...
                      <div>
                        <span className="text-gray-600">Is Renewed:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.is_renewed, "boolean")}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-600">Renewal Count:</span>{" "}
                        <span className="ml-1 text-gray-900">
                          {displayValue(previewData.renewal_count)}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex justify-between gap-4 pt-6 border-t border-gray-200">
                  <div className="flex gap-3 items-center">
                    ...
                    >
                      <i className="fas fa-arrow-left mr-2"></i>Go Back to Edit
                    </button>
                    ...
                    {isEditMode && savedEmploymentId && (
                      ...
                      >
                        <i className="fas fa-clone mr-2"></i>Save as new record
                      </button>
                    )}
                  </div>
                  <button
