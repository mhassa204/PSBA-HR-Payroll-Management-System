...
const PDFPreview = ({
  ...
  if (error) {
    return (
      <div
        className={`flex flex-col items-center justify-center bg-gray-50 border border-gray-200 rounded ${className}`}
        style={{ height }}
      >
  ...
  const isBlobUrl = url && url.startsWith("blob:");
  const pdfUrl = `${url}#toolbar=${
    showControls ? 1 : 0
  }&navpanes=0&scrollbar=1`;

  return (
    <div
      className={`relative border border-gray-200 rounded overflow-hidden bg-gray-50 ${className}`}
      style={{ height }}
    >
