...
router.get(
  "/employees/reportees",
  isAuthenticated,
  travelRequestController.reportees
);
router.get(
  "/employees/search",
  isAuthenticated,
  travelRequestController.searchEmployees
); // New route for employee search
// New: reportees of a specific employee (used by manual entry after selecting applicant)
router.get(
  "/employees/:id/reportees",
  isAuthenticated,
  travelRequestController.reporteesOfApplicant
);
...
router.get(
  ...
  isAuthenticated,
  travelRequestController.listPendingApprovals
);

// Recommender actions
router.post(
  "/requests/:id/recommend",
  isAuthenticated,
  travelRequestController.recommend
);
router.post(
  "/requests/:id/recommend/clear",
  isAuthenticated,
  travelRequestController.clearRecommendation
);
router.post(
  "/requests/:id/recommend/decision",
  isAuthenticated,
  travelRequestController.recommendDecision
);
...
router.put(
  "/requests/:id",
  isAuthenticated,
  authorizeAny(["travel.manage", "travel.update"]),
  travelRequestController.update
);
...
router.post(
  ...
  isAuthenticated,
  travelRequestController.legacyDecision
);
router.post(
  "/requests/:id/clear-last",
  isAuthenticated,
  travelRequestController.clearLast
);
...
router.get(
  "/me/capabilities",
  isAuthenticated,
  travelRequestController.capabilities
);
...
router.patch(
  "/requests/:id/status",
  isAuthenticated,
  travelRequestController.updateStatusFlexible
);
...
router.post(
  "/requests/manual",
  isAuthenticated,
  travelRequestController.manualCreate
);
// Record recommendation/approval with selected actor identity
router.post(
  "/requests/:id/manual/decision",
  isAuthenticated,
  travelRequestController.manualDecision
);
...
router.get(
  ...
  isAuthenticated,
  expenseClaimController.eligible
);
router.get(
  "/expense-claims/pending-approvals",
  isAuthenticated,
  expenseClaimController.listPendingApprovals
);
router.get(
  "/expense-claims/all",
  isAuthenticated,
  expenseClaimController.listAll
);
// Accounts processing: filter verified claims and create tranches (place BEFORE dynamic :id routes)
router.get(
  "/expense-claims/accounts",
  isAuthenticated,
  expenseClaimController.listForAccounts
);
router.post(
  "/expense-claims/tranches",
  isAuthenticated,
  expenseClaimController.createTranche
);
router.get(
  "/expense-claims/tranches",
  isAuthenticated,
  expenseClaimController.listTranches
);
router.get(
  "/expense-claims/tranches/:id/export",
  isAuthenticated,
  expenseClaimController.exportTranche
);
// Proxy documents for claims
router.get(
  "/expense-claims/document",
  isAuthenticated,
  expenseClaimController.docProxy
);
...
// Relax delete guard: controller/service enforce business rules (own/reportee/department scope and DRAFT only)
router.delete(
  "/expense-claims/:id",
  isAuthenticated,
  expenseClaimController.delete
);

// Segments
router.post(
  "/expense-claims/:id/segments",
  isAuthenticated,
  expenseClaimController.addSegment
);
router.put(
  "/expense-claims/:id/segments/:segmentId",
  isAuthenticated,
  expenseClaimController.updateSegment
);
router.delete(
  "/expense-claims/:id/segments/:segmentId",
  isAuthenticated,
  expenseClaimController.deleteSegment
);

// Documents upload: allow any category (FUEL, TOLL, PICTURE, REPORT, OTHER); supports multi-file upload
router.post(
  "/expense-claims/:id/documents",
  isAuthenticated,
  expenseClaimController.uploadDocuments
);
router.delete(
  "/expense-claims/:id/documents/:docId",
  isAuthenticated,
  expenseClaimController.deleteDocument
);
...
router.post(
  "/expense-claims/:id/decision",
  isAuthenticated,
  expenseClaimController.decide
);
