...
const employmentService = {
  ...
  mapLocationCompat(loc) {
    ...
    return {
      ...
      city:
        typeof loc.city === "object" && loc.city !== null
          ? loc.city.name
          : null,
      district:
  ...
  checkExistingRenewalReport: async (employmentId) => {
    try {
      ...
      const renewalDocuments = allDocuments.filter(
        (doc) => doc.file_type === "renewal_report"
      );

      return renewalDocuments.length > 0 ? renewalDocuments[0] : null;
    } catch (error) {
      console.error(
        `❌ EmploymentService: Error checking for renewal report documents:`,
        error
      );
      return null;
    }
  ...
  buildRenewalDocFromMeta: async (tx, employmentId, data) => {
    try {
      ...
      const hasMeta =
        data.renewal_report_file_path ||
        data.renewal_report_document_name ||
        data.renewal_report_url;
      ...
      if (hasMeta) {
        source = {
          file_path:
            data.renewal_report_file_path ||
            (typeof data.renewal_report_url === "string"
              ? data.renewal_report_url.replace(/^\/uploads\//, "uploads/")
              : null),
          document_name:
            data.renewal_report_document_name || "Contract Renewal Report",
          mime_type: data.renewal_report_mime_type || "application/pdf",
          file_size: data.renewal_report_file_size
            ? parseInt(data.renewal_report_file_size, 10)
            : null,
        };
      } else if (hasId) {
        const existing = await tx.employmentDocument.findFirst({
          where: {
            id: parseInt(data.renewal_report_id, 10),
            is_deleted: false,
          },
        });
        if (existing) {
      ...
      console.warn(
        "⚠️ EmploymentService: Failed to build renewal doc from metadata:",
        e.message
      );
      return null;
    }
  ...
  resolveLocationId: async (tx, loc) => {
    ...
    if (
      ...
      if (!Number.isNaN(numeric)) {
        const existingById = await tx.location.findFirst({
          where: { id: numeric, is_deleted: false },
        });
    ...
    if (
      ...
      [
        ...
        "HEADQUARTER",
        "OFFICE",
      ].includes(String(rawName).toUpperCase())
    ) {
    ...
    if (!district_id && loc.district) {
      const d = await tx.district.findFirst({
        where: { name: loc.district, is_deleted: false },
      });
      district_id = d?.id || null;
    }
    if (!city_id && loc.city) {
      const c = await tx.city.findFirst({
        where: { name: loc.city, is_deleted: false },
      });
      city_id = c?.id || null;
    }
  ...
  createEmployment: async (data) => {
    ...
    const possibleLocId =
      filteredData.location_id ||
      filteredData.location_location_id ||
      filteredData["location_location_id"] ||
      filteredData["location[location_id]"] ||
      filteredData.locationId;
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (location) {
        resolvedLocationId = await employmentService.resolveLocationId(
          tx,
          location
        );
      }
      ...
      const hasUploadedRenewal =
        Array.isArray(documentRecords) &&
        documentRecords.some((d) => d.file_type === "renewal_report");
      if (!hasUploadedRenewal) {
        await employmentService.buildRenewalDocFromMeta(
          tx,
          employment.id,
          filteredData
        );
  ...
  getEmploymentById: async (id) => {
    ...
    const processedDocuments = employment.documents.map((doc) => {
      ...
      return {
        ...
        fileExtension: doc.file_path
          .substring(doc.file_path.lastIndexOf(".") + 1)
          .toLowerCase(),
      };
    });
  ...
  cloneEmployment: async (sourceEmploymentId, overrides = {}) => {
    ...
    return prisma.$transaction(async (tx) => {
      const cloned = await tx.employment.create({
        data: {
          ...
          effective_from: overrides.effective_from
            ? new Date(overrides.effective_from)
            : src.effective_from,
          effective_till: overrides.effective_till
            ? new Date(overrides.effective_till)
            : src.effective_till,
          ...
          remarks:
            overrides.remarks !== undefined ? overrides.remarks : src.remarks,
      ...
      if (src.salary) {
        await tx.employmentSalary.create({
          data: {
            employment_id: cloned.id,
            basic_salary: src.salary.basic_salary,
            gross_salary: src.salary.gross_salary,
            medical_allowance: src.salary.medical_allowance,
            house_rent: src.salary.house_rent,
            conveyance_allowance: src.salary.conveyance_allowance,
            other_allowances: src.salary.other_allowances,
            daily_wage_rate: src.salary.daily_wage_rate,
            bank_account_primary: src.salary.bank_account_primary,
            bank_name_primary: src.salary.bank_name_primary,
            bank_branch_code: src.salary.bank_branch_code,
            payment_mode: src.salary.payment_mode,
            salary_effective_from: src.salary.salary_effective_from,
            salary_effective_till: src.salary.salary_effective_till,
      ...
      if (src.contract) {
        await tx.employmentContract.create({
          data: {
            employment_id: cloned.id,
            contract_type: src.contract.contract_type,
            contract_number: src.contract.contract_number,
            start_date: src.contract.start_date,
            end_date: src.contract.end_date,
            renewal_count: src.contract.renewal_count,
            probation_start: src.contract.probation_start,
            probation_end: src.contract.probation_end,
            confirmation_status: src.contract.confirmation_status,
            confirmation_date: src.contract.confirmation_date,
            is_renewed: src.contract.is_renewed,
          },
        });
      }
      ...
      if (src.documents && src.documents.length) {
        ...
        const renewalDoc = await tx.employmentDocument.findFirst({
          where: {
            employment_id: src.id,
            file_type: "renewal_report",
            is_deleted: false,
          },
        });
        if (renewalDoc) {
          await tx.employmentDocument.create({
            data: {
              employment_id: cloned.id,
              file_path: renewalDoc.file_path,
              file_type: renewalDoc.file_type,
              document_name: renewalDoc.document_name,
              file_size: renewalDoc.file_size,
              mime_type: renewalDoc.mime_type,
              associated_id: renewalDoc.associated_id,
            },
          });
        }
      }
      ...
      return {
        ...full,
        location: employmentService.mapLocationCompat(full.location),
        documents: processedDocuments,
      };
    });
  },
  ...
  updateEmployment: async (id, data) => {
    ...
    const changedByInt = changedBy ? parseInt(changedBy) : null;

    return prisma.$transaction(async (tx) => {
      let resolvedLocId = null;
      if (location) {
        resolvedLocId = await employmentService.resolveLocationId(tx, location);
      }
      const employmentUpdateData = {};
      if (organization !== undefined)
        employmentUpdateData.organization = organization;
      if (department_id !== undefined)
        employmentUpdateData.department_id = department_id
      ...
      if (salary) {
        ...
        await tx.employmentSalary.upsert({
          ...
          update: salaryData,
          create: { employment_id: employment.id, ...salaryData },
        });
      }
      ...
      if (contract) {
        ...
        await tx.employmentContract.upsert({
          where: { employment_id: employment.id },
          update: contractData,
          create: { employment_id: employment.id, ...contractData },
        });
      }
      ...
      // Track history changes - do this after all updates are complete
      const oldData = {
        department_id: currentEmployment.department_id,
        designation_id: currentEmployment.designation_id,
        role_tag_id: currentEmployment.role_tag_id,
        scale_grade_id: currentEmployment.scale_grade_id,
        location_id: currentEmployment.location_id,
        employment_type: currentEmployment.employment_type,
        employment_status: currentEmployment.employment_status,
        is_current: currentEmployment.is_current,
        filer_status: currentEmployment.filer_status,
        filer_active_status: currentEmployment.filer_active_status,
        is_on_probation: currentEmployment.is_on_probation,
        probation_end_date: currentEmployment.probation_end_date,
        reporting_officer_id: currentEmployment.reporting_officer_id,
        office_location: currentEmployment.office_location,
        remarks: currentEmployment.remarks,
        effective_from: currentEmployment.effective_from,
        effective_till: currentEmployment.effective_till,
        salary: currentEmployment.salary || {},
        contract: currentEmployment.contract || {},
      };

      // Get updated salary and contract data
      const updatedSalary =
        (await tx.employmentSalary.findUnique({
          where: { employment_id: employment.id },
        })) || {};
      ...
      await employmentHistoryService.trackEmploymentChanges(
        tx,
        parseInt(id),
        oldData,
        newData,
    ...
    return prisma.employmentSalary.create({
      data: {
        ...
        medical_allowance: salaryData.medical_allowance
          ? parseFloat(salaryData.medical_allowance)
          : 0,
        house_rent: salaryData.house_rent
          ? parseFloat(salaryData.house_rent)
          : 0,
        conveyance_allowance: salaryData.conveyance_allowance
          ? parseFloat(salaryData.conveyance_allowance)
          : 0,
        other_allowances: salaryData.other_allowances
          ? parseFloat(salaryData.other_allowances)
          : 0,
        daily_wage_rate: salaryData.daily_wage_rate
          ? parseFloat(salaryData.daily_wage_rate)
          : null,
        ...
        salary_effective_from: salaryData.salary_effective_from
          ? new Date(salaryData.salary_effective_from)
          : null,
        salary_effective_till: salaryData.salary_effective_till
          ? new Date(salaryData.salary_effective_till)
          : null,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (locationData.location_id || locationData.id) {
        const existing = await tx.location.findFirst({
          where: {
            id: Number(locationData.location_id || locationData.id),
            is_deleted: false,
          },
        });
  ...
  getEmploymentStatistics: async () => {
    const [totalRecords, currentEmployees, byOrganization, byDepartment] =
      await Promise.all([
