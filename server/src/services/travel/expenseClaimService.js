...
module.exports = {
  ...
  createClaim: async (employee_id, data) => {
    ...
    // unique constraint will enforce uniqueness, but pre-check:
    ...
    return prisma.travelClaim.findUnique({ where: { id: created.id }, include: { documents: true, segments: true } });
  },
  getClaim: async (id, employee_id, isSuperAdmin) => {
    const claim = await prisma.travelClaim.findUnique({ where: { id: Number(id) }, include: { documents: true, segments: true } });
    if(!claim || claim.is_deleted) return null;
    if(!isSuperAdmin && claim.employee_id !== employee_id) throw new Error('Forbidden');
    return claim;
  },
  updateClaim: async (id, employee_id, isSuperAdmin, data) => {
    const claim = await prisma.travelClaim.findUnique({ where: { id: Number(id) } });
    ...
    // recompute formulas later after segments update
  ...
  recomputeTotals: async (claimId) => {
    const claim = await prisma.travelClaim.findUnique({ where: { id: Number(claimId) }, include: { segments: true, documents: true } });
    ...
    return prisma.travelClaim.findUnique({ where: { id: claim.id }, include: { documents: true, segments: true } });
  },
  addSegment: async (claimId, employee_id, isSuperAdmin, payload) => {
    const claim = await prisma.travelClaim.findUnique({ where: { id: Number(claimId) } });
    if(!claim || claim.is_deleted) throw new Error('Not found');
    if(!isSuperAdmin && claim.employee_id !== employee_id) throw new Error('Forbidden');
  ...
  addDocuments: async (claimId, employee_id, isSuperAdmin, files, category) => {
    const claim = await prisma.travelClaim.findUnique({ where: { id: Number(claimId) }, include: { documents: true } });
    if(!claim || claim.is_deleted) throw new Error('Not found');
    if(!isSuperAdmin && claim.employee_id !== employee_id) throw new Error('Forbidden');
